...
def generate_html_report(reportData):
    ...
    inventoryDetails = reportData["inventoryDetails"]
 ...
 #---------------------------------------------------------------------------------------------------
    ...
    html_ptr.write("            <th style='width: 10%' class='text-center'>CREATED DATE</th>\n")
    html_ptr.write("            <th style='width: 15%' class='text-center'>CREATED BY</th>\n")
    html_ptr.write("            <th style='width: 15%' class='text-center'>LAST UPDATED DATE</th>\n")   
    html_ptr.write("            <th style='width: 15%' class='text-center'>LAST UPDATED BY</th>\n") 
    html_ptr.write("        </tr>\n")
    html_ptr.write("    </thead>\n") 
    html_ptr.write("    <tbody>\n")  

    for inventoryItemID in inventoryDetails:
        inventoryItemName =  inventoryDetails[inventoryItemID]["inventoryItemName"]
        inventoryItemLink =  inventoryDetails[inventoryItemID]["inventoryItemLink"]
        projectName =  inventoryDetails[inventoryItemID]["projectName"]
        projectLink =  inventoryDetails[inventoryItemID]["projectLink"]
        createdOn = inventoryDetails[inventoryItemID]["createdOn"]
        createdBy = inventoryDetails[inventoryItemID]["createdBy"]
        createdByEmail = inventoryDetails[inventoryItemID]["createdByEmail"]
        updatedOn = inventoryDetails[inventoryItemID]["updatedOn"]
        updatedBy = inventoryDetails[inventoryItemID]["updatedBy"]
        updatedByEmail = inventoryDetails[inventoryItemID]["updatedByEmail"]

        if len(projectList) > 1:
            html_ptr.write("<td style=\"vertical-align:middle\"><a href=\"%s\" target=\"_blank\">%s</a></td>\n" %(projectLink, projectName))

        html_ptr.write("<td style=\"vertical-align:middle\"><a href=\"%s\" target=\"_blank\">%s</a></td>\n" %(inventoryItemLink, inventoryItemName))
        html_ptr.write("<td style=\"vertical-align:middle\">%s</td>\n" %createdOn)
        if createdByEmail:
            html_ptr.write("<td style=\"vertical-align:middle\"><a href=mailto:\"%s\">%s</a></td>\n" %(createdByEmail, createdBy))
        else:
            html_ptr.write("<td style=\"vertical-align:middle\">%s</td>\n" %createdBy)
        html_ptr.write("<td style=\"vertical-align:middle\">%s</td>\n" %updatedOn)
        if updatedByEmail:
